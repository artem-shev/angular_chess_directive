(function () {
	...
	angular.module('chessApp.common')
		...
		boardService.$inject = ['$log'];

		function boardService ($log) {
			var service = {
				constructor: Board
			};
			...
			Board.prototype.validateMove = function (selectedCells) {
				...
				var moveTurn = (validation.startCell.figure.isBlack && self.whiteTurn) || (!validation.startCell.figure.isBlack && !self.whiteTurn);
				if (moveTurn) {return;}
...
//protect	
			...
			Board.prototype.checkGameOverStates = function () {
				var self = this;											
//check draw state (insufficient material)
				if (self.figures.length <= 4) {
					...
					if (drawCheck[0] && drawCheck[1]) {
						...
						$log.log('game over', self.gameStatus, drawCheck)
						return;
					}
				}
...
//check if there any available move
				...
				if (availableMoves.length === 0) {
					if (isCaptured) {
						...
						$log.log('game over', self.gameStatus)
						return;
					} else {
						self.gameStatus = {
							gameOver: true,
					...
					if (iRow === rows.length - 1) {
						...
						self.players.forEach(function (player) {
							...
							if (king.firstMove) {
								...
								if (rightRook && rightRook.firstMove) {
									castlings += 'k'; 
								}
								if (leftRook && leftRook.firstMove) {
									castlings += 'q'
								}
								if (!player.isBlack) {
